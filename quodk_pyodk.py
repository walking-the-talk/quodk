...
#            self.dlg.msg.setStyleSheet("background: red")
        ...
        self.dlg.clearFilter.clicked.connect(self.updateTable)
        self.dlg.crs.setEnabled(False)
        self.dlg.crs.CrsOption(1)
#        self.dlg.crs.setNotSetText('Use default projection')
        self.dlg.crs.crsChanged.connect(self.crs)
        self.dlg.save_csv.clicked.connect(self.tunzaCSV)
...
#        self.dlg.QGIS_Layer_label.setText('Load ODK submissions to QGIS')
        ...
        self.tr_layer_added = "Layer added"
        ...
        self.dlg.help_text.setText('<h4>About</h4><p>This plugin works with a connection to an ODK Central Server. It allows you to load geo-located data from ODK Collect forms as QGIS temporary layers, as well as download associated attachments. It is designed to allow you to incrementally download data, although you can, with caution, opt to load all records.</p><h4>Set up</h4><p>Ask the person responsible for managing your ODK Central server to provide you with log-in details (<strong>hint:</strong> it may be useful to set up a separate <a href="https://docs.getodk.org/central-users/">web-user on Central</a> with restricted access - e.g. Project Viewer). You can choose to save the password (stored in plain-text on your computer in the plug-in folder).</p><h4>Loading data</h4><p>Once you connect to your server you will get a list of ODK projects if appropriate, and then you can select the form you would like to download. If your form has repeat groups with location data you can select the repeat, or use the main form. You can load any form or repeat as a simple table if there is no location data (in case you want to link data in QGIS).</p><p>The plugin converts field names as close to that used by ODK Central when you download directly - you can choose to remove group headings too. You can also download data as CSV.</p><h4>Attachments</h4><p>After you have selected the data to download you can view or download any attachments. Be aware that the plugin might freeze for a while when it loads the attachments form if you have lots of submissions. If you choose to download all the attachments listed it can take a while needing high bandwidth - use with caution. You can choose a folder on your computer to download all attachments.</p><h4>Credits</h4><p>The original plugin was written in Swahili for WWF Tanzania by	<strong>Cuthbert-Langen Mushi</strong>. Additional features, code refactoring and translations were implemented by <a href="https://walking-the-talk.co.uk">Walking-the-Talk</a> - any translation errors lie with Chris York (and maybe Google Translate) in trying to respect the original author and language.</p><h4>Find out more</h4><p><a href="https://getodk.org">ODK website</a></p><p><a href="https://forum.getodk.org">ODK forum</a> and <a href="https://docs.getodk.org">ODK documentation</a></p>')
    ...
    def get_logins(self):
        quodkinit = os.path.join(os.path.dirname(__file__), '.pyodk_config.toml')
            
        if os.path.isfile(quodkinit):
            fileconf=toml.load(quodkinit)
            self.dlg.odk_url.setText(fileconf[base_url])
            self.dlg.odk_username.setText(fileconf[username])
            self.dlg.odk_pass.setText(fileconf[password])
            '''f = open(quodkinit, 'r')
            credentials = f.readlines()
            if credentials[0] = "[central]":
#                print(credentials)
                credentials_split = (credentials).split("\n")
                f.close()
            
                odk_url_toml = (credentials[1]).split(" = ")
                if odk_url_toml[0] == 'base_url':
                    odk_url_tomlb = str(odk_url_toml[1])
                    
                
                odk_username_toml = (credentials_split[2]).split(" = ")
                if odk_username_toml[0] == 'username':
    ...
    def set_logins(self):
        ...
        if os.path.isfile(quodkinit):
            fileconf=toml.load(quodkinit)
            if not equal(conf, fileconf):
                file=open(quodkinit,"w")
                toml.dump(conf,file)
                file.close()
        ...
        if not os.path.isfile(quodkinit):
            ...
            toml.dump(conf,file)
            file.close()
        '''odk_url_odk_username = '[central]\nbase_url = \\"' + str(odk_url) + '"\n' + 'username = "' + str(odk_username)+ '"\npassword = "'  + str(odk_pass)+'"\ndefault_project_id = '+ str(self.dlg.projectID.currentText()).split('|')[0]
    ...
    def connectODK(self): #triggered by Connect button on either tab
        ...
        global client
        client = Client(os.path.dirname(__file__),os.path.dirname(__file__))
...
#            print(formID)
    ...
    def list_repeatGroup(self): # triggered by select form combo box
        ...
        # Get sub forms
        svc = "projects/"+str(projectID)+"/forms/"+str(iformID)+".svc"
        subForms = client.session.get(svc)
        subFormID = subForms.json()['value']
    ...
    def get_submissions(self):
        #(re)set dependent combo-boxes to clear 
        ...
        if  sDate > eDate:  #check dates are a real range
            ...
            filtered = filterDateS+" and "+filterDateE
            odkjson = client.submissions.get_table(form_id=iformID,project_id=selected_projectID, table_name='Submissions', filter = filtered )["value"]
            

            #global dataframe for the (normalised) Json data retrieved
            global df_ODK
            df_ODK = pd.json_normalize(SubJsonAll)      #create the 'master' dataframe
            df_ODK = df_ODK.assign(geometry=None)
            keyID = list(df_ODK['__id'])
            self.dlg.msg.setText(self.tr_submissions_found + str(len(keyID)))
            
            if len(keyID) == 0:
                self.dlg.msg.setStyleSheet("background: orange")
            else:
                self.dlg.msg.setStyleSheet("background: cyan")
                self.updateTable(df_ODK)
...
#        try:
        ...
        '''if  sDate > eDate:  #check dates are a real range
            ...
            filtered = filterDateS+" and "+filterDateE
            odkjson = client.submissions.get_table(form_id=iformID,project_id=projectID, table_name='Submissions', filter = filtered )["value"]


        
            for iData in odkJson:
#                    print(iData)
                submissionKEY = iData['__id']
                print(submissionKEY,)
                keyID.append(str(submissionKEY))
            self.dlg.msg.setText(self.tr_submissions_found + str(len(odkJson)))
        ...
        #If there are no repeats, list the main form
...
#            self.dlg.msg.setStyleSheet("background: red")
    ...
    def list_submissions(self): # triggered by repeatGroup combo or if there are no repeats in the form
        ...
        if sel_repeatGroup == self.tr_mainForm: # either get submissions from within the date range (if main form)
            iSub = str(sel_repeatGroup).split("(")[0]

            keyLevel = '__id'
        else: #  or all rows from subform (will be filtered below)
            filterURL = (form_url,str(iformID),".svc/",str(sel_repeatGroup),"?$filter=$root/Submissions/",filterDateS," and $root/Submissions/",filterDateE)
            keyLevel = '__Submissions-id'
        print("".join(filterURL))
        SubResponse = requests.get("".join(filterURL), headers={"Authorization": "Bearer " + session_token},)          

        if SubResponse.status_code == 200:

            SubJsonAll = SubResponse.json()['value']
            print(SubJsonAll)
            df_ODK = pd.json_normalize(SubJsonAll)      #create the 'master' dataframe
            df_ODK = df_ODK.assign(geometry=None)
            if self.dlg.load_all.isChecked():
                pass
                 ...
                 print(df_ODK[keyLevel], keyID)
                 df_ODK = df_ODK[df_ODK[keyLevel].isin(keyID)]
            ...
            headings = list()
            col_number = 0
#            df_Attachments =df_ODK.filter(regex='attachmentsPresent')
#            AttachmentsTotal = df_Attachments.sum()
            AttachmentsTotal = df_ODK['__system.attachmentsPresent'].sum()
            #loop through column names and update (attempt to get the same pattern as Central download CSV)
            for header in headers : 
                if header.find('.') >-1:
                    # look for geometry field (name, type, accuracy) and replace column name
#                    geomType = df_ODK.loc[0,header]
#                    print(header, geomType)
#if df_ODK[header].str.isin(geomkeys).any():
                    ...
                    print(header, df_ODK.loc[0,header])
                    if df_ODK.loc[0,header] in (geomKeys):
                        geomType = df_ODK.loc[0,header]
...
#                self.dlg.odk_geometry.addItem(str(geomKey)
            self.updateTable(df_ODK)    #load the dataframe to the Qtable

 
            self.dlg.save_csv.setEnabled(True)
            self.dlg.load_to_canvas.setEnabled(True)  
            self.dlg.Attachments.setEnabled(True)
            if self.dlg.repeatGroup == self.tr_mainForm:
                self.dlg.filterValue.setEnabled(True)
                self.dlg.attributeFilter.setEnabled(True)
            self.dlg.msg.setText('Attachments found: '+str(AttachmentsTotal))
    ...
    def filter_items(self): # triggered by attributeFilter combo
        ...
        if attributeFilter != self.tr_attributeFilter:
            uniqueValues = df_ODK[attributeFilter].unique().tolist()
#            print(uniqueValues)
            self.dlg.filterValue.setEnabled(True)
    ...
    def filter_Value(self): #triggered by filterValue combo
...
#        self.dlg.filterValue.clear()
        ...
        if filterValue == self.tr_filterValues:
            df_filter = df_ODK
        else:
            df_filter = df_ODK[df_ODK[attributeFilter] == filterValue]
        print(filterValue, df_filter.shape, df_ODK.shape)
        self.updateTable(df_filter)
    ...
    def updateTable(self, df_table):
        ...
        global filterID
        filterID = list(df_table['KEY'])
#        print (filterID)
#        df_Attachments =df_ODK.filter(regex='attachmentsPresent')
        AttachmentsTotal = df_table['__system.attachmentsPresent'].sum()
        self.dlg.msg.setText('Attachments found: '+str(AttachmentsTotal))
        return None
    ...
    def layerLoadQgis(self):
        ...
        df_layer = df_layer[df_layer['KEY'].isin(filterID)]
#        print(df_layer.head())
...
#            try:
            if str(layerGeom) == "NoGeometry": 
                ...
                LayerType = str(str(layerGeom)+"?crs=epsg:4326")
            
            if sel_repeatGroup == self.tr_mainForm:
                result = FormName
            else:
                result = FormName+'_'+sel_repeatGroup.split(".")[1]
...
#            print(headers)
            ...
            for i,iData in df_layer.iterrows():  
                ...
                if str(layerGeom) == "Point":                    
                    ...
                    if type(iValues) is list:
                        ...
                        iXYZ = ("POINT (" + XYZ +")")                            
                        df_layer.loc[i, 'geometry'] = str(iXYZ)
                    
                #--------------------------------------------------------
                #Convert to Linestring or Polygon WKT
                if  str(layerGeom) == "LineString" or str(layerGeom)  == "Polygon":                           
                    iValues = df_layer.loc[i, geomCol] 
                    if type(iValues) is list:                        
                        c = ""
                        for a in iValues:
            ...
            #processing.run("native:reprojectlayer", {'INPUT':temp,'TARGET_CRS':crs,'OPERATION':'crs_operation','OUTPUT':'memory:'})
            self.dlg.msg.setText(self.tr_layer_added)
...
#            self.dlg.msg.setStyleSheet("background: red")
    ...
    def download_images(self):                    
        ...
        odk_username = self.dlg.odk_username.text()
        odk_pass = self.dlg.odk_pass.text()
#        projectID = self.dlg.projectID.currentText().split('|')[0]
        formID = self.xmlFormId[self.dlg.formID.currentIndex()]
#        DateStart = self.dlg.DateStart.date()
#        DateEnd = self.dlg.DateEnd.date()
#        iattributeFilter = self.dlg.attributeFilter.currentText()
#        sel_repeatGroup = self.dlg.repeatGroup.currentText()
        df_attach = df_attach[df_attach['KEY'].isin(filterID)]
        AttachmentsTotal = df_attach['__system.attachmentsPresent'].sum()
        print(df_attach.head())
        count = 0
        #--------------------------------------------------------
        aFolder = QFileDialog.getExistingDirectory(None, self.tr_image_folder)
        if aFolder:
           # self.dlg.msg.setText(str(aFolder))
...
#----------------
    ...
    # ====================================================================================================
    # Export XSV
    def shusha_csv(self,odk_usernameFaili): 
        if sel_repeatGroup == self.tr_mainForm:
            keyLevel = '__id'
        else:
            keyLevel = '__Submissions-id'
...
#           manage column names - make consistent with Central output
        if len(df.index) > 0:
            headers = df.columns.tolist()
            col_number = 0
            for header in headers : 
                datatype= df[header].dtype
                if header.find('.') >-1:
                    if self.dlg.remove_groups.isChecked():
                        #geom_headings (coordinates, type and accuracy)
                        if header.split('.')[0] == str(geomName):
                            header = header.replace('.','-')
                            header = header.replace('-properties','') # in the case of Point geometry to extract Accuracy
                        else:
                            header = header.split('.')[1]
                    else:
                        header = header.replace('.','-')
                header = header.replace('__Submissions-id','PARENT_KEY')
                header = header.replace('__id','KEY')
                df.rename(columns={df.columns[col_number]: header}, inplace=True)     
                col_number+=1
            df.to_csv(odk_usernameFaili_OUT)  
            
            lFaili = 'file:///{}?delimiter=,'.format(odk_usernameFaili_OUT)
            mFaili = os.path.splitext(os.path.basename(odk_usernameFaili_OUT))[0]
        ...
        # ================================================   
        self.dlg.msg.setText(odk_usernameFaili)
        self.dlg.msg.setStyleSheet("background: blue")
    
    #-----------------------------------------------------------------------------------------------------
    def tunzaCSV(self):
        iformID = self.xmlFormId[self.dlg.formID.currentIndex()]
        odk_usernameFaili, odk_geometryFaili = QFileDialog.getSaveFileName(None,'Tuanza attributeFilter za ' + iformID,iformID,"*.csv")        
        if odk_usernameFaili:
            self.shusha_csv(odk_usernameFaili)
            
        
    #-----------------------------------------------------------------------------------------------------
    def tunzaViambatanisho(self):
        self.dlg.msg.setText(self.tr_wait_images)
#        self.dlgI.show()        
        self.dlg.orodhaViambata.clear()
        self.dlg.orodhaViambata.setColumnCount(3)
        self.dlg.orodhaViambata.setHeaderLabels(self.tr_treewidget_head)
        self.df_attachments = pd.DataFrame(columns = ['instanceID','odk_filename','odk_type','odk_url'])  #flat dataframe of form attachments within date range      
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        iodk_url = self.dlg.odk_url.text()        
        selected_projectID = self.dlg.projectID.currentText().split('|')[0]
        iformID = self.xmlFormId[self.dlg.formID.currentIndex()]
        DateStart = self.dlg.DateStart.date()
        DateEnd = self.dlg.DateEnd.date()
        iattributeFilter = self.dlg.attributeFilter.currentText()
        ...
        totalimages = 0
        #--------------------------------------------------------
        ##########Pata attributeFilter za Zilizochaguliwa kwa Tarehe (Find selected submissions by date)
        sDate = DateStart.toPyDate()
        eDate = DateEnd.toPyDate()
        filterDateS = "__system/submissionDate ge " + str(sDate)+'T00:00:00.0Z'
        filterDateE = "__system/submissionDate le " + str(eDate)+'T23:59:59.99Z'
        jattributeFilter = str(iattributeFilter)
        ...
        try:
            if  sDate > eDate:
                ...
                filterURL = (str(iodk_url),"/v1/projects/",str(selected_projectID),"/forms/",str(iformID),".svc/Submissions?$filter=",filterDateS," and ",filterDateE)
#                odk_urlFoo = "".join(filterURL)                
            #--------------------------------------------------------
            iodk_url_Response = requests.get("".join(filterURL), headers={"Authorization": "Bearer " + session_token},)  
#            print(iodk_url_Response)
            if iodk_url_Response.status_code == 200:
                odkJson = iodk_url_Response.json()['value']
                ...
                for iData in odkJson:
                    ...
                    InstanceID = [jData['instanceID']]
                    jodk_url = (str(iodk_url),"/v1/projects/",str(selected_projectID),"/forms/",str(iformID))
                    jodk_url = "".join(jodk_url)     
                    iKiambata = (jodk_url,"/submissions/",str(jData['instanceID']),"/attachments")
                    iKiambata = "".join(iKiambata) 
                    
                    jodk_url_Response = requests.get(iKiambata, headers={"Authorization": "Bearer " + session_token}, )                                        
                    if jodk_url_Response.status_code == 200:
                        jJson = jodk_url_Response.json()
                        ...
                        for kData in jJson:
                            ...
                            item.addChild(child)
                            #NewRow = pd.DataFrame({'instanceID': InstanceID,'odk_filename': lData,'odk_type': mData,'odk_url': nData})
                            #self.df_attachments.append(NewRow, ignore_index = True) 
                            totalimages+=1

                        items.append(item)
                        
#                print(self.df_attachments.head())       
                self.dlg.orodhaViambata.insertTopLevelItems(0, items)
